...
with the action for it (§GRUND-001-overseer.2). Success is observable at the
...
the matter anywhere else first (§GRUND-001-overseer.1).

## GOAL-003-nothing-lost: the watch is trusted enough to retire the sweep

The hand-kept morning sweep (§GRUND-001-overseer.1) stops only when the tool
...
(§GRUND-001-overseer.2). What came of the handover returns to the row.
...
(§GRUND-001-overseer.3). Success is observable at both edges: tracking a new
...
site, or fused vendor named in what ships (§GRUND-001-overseer.2).
