...
The engine that realizes §FS-008-attribution is pure matching: it takes
...
pipeline's attribute stage (§AR-008-pipeline) and nowhere else — no source
places its own items, a split decided with its configuration cost in
§DA-002-fetch-attribution-split.
...
where the row does not override (§FS-008-attribution.1) — identity lives in
the row per the three homes (§REQ-001-boundary.2): ticket patterns,
...
matter on a project and branch. Both apply §FS-008-attribution.3: an
...
(§FS-008-attribution.4). Branch matching inside a project is the same
