...
The engine that realizes [§FS-008-attribution](../../requirements.md#fs-008-attribution-every-conversation-finds-its-project-or-says-that-it-could-not) is pure matching: it takes
...
where the row does not override ([§FS-008-attribution.1](../../requirements.md#1-identity-is-declared-and-the-row-has-the-last-word)) — identity lives in
...
matter on a project and branch. Both apply [§FS-008-attribution.3](../../requirements.md#3-venue-beats-reference-beats-resemblance): an
...
([§FS-008-attribution.4](../../requirements.md#4-unattributed-is-a-place-not-a-fate)). Branch matching inside a project is the same
