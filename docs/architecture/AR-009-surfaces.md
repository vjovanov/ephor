...
API and nothing else (§AR-001-layers.1). It may choose what to show, how to
...
- **A reading** takes what it is asked about and returns a view — a plain
  ...
  it reads as an oversight (§REQ-001-boundary.1); and so are the two that reach a
...
- **A move** takes a request and returns an outcome — a plain serializable
  ...
  has to reach a greyed menu row and a JSON field (§AR-005-capabilities.2).
...
prints it as prose or as JSON (§REQ-002-parity.3). That holds for every move
...
larger (§FS-005-dispatch.19), then hands what it collected to the same call
...
project is placed (§AR-004-forest.3), what each project can do
(§AR-005-capabilities), the branch standings, the configured entries, the
...
and printable by name (§FS-006-project-interface.11), and the schemas are the
stability surface that §REQ-002-parity.4 promises: what a release may change
...
§E2E-012-command-line-parity runs every one of them and validates its output
against the published subschema, and its table of what it swept is checked
against the shape list in both directions, so a shape nobody validates fails
rather than passing quietly. A declared shape nobody validates against is a
declaration; §REQ-002-parity.4 promises a contract.
...
`views.rs` is data and pure functions, which is where §AR-001-layers.1 puts
it. The rest is engine: it reads the cache, resolves bindings, and calls the
one executor (§AR-002-summons) for anything that reaches the world.
...
where an entry runs (§AR-002-summons.1).

## 5. The seam between them is checked

The ability list of §REQ-002-parity.5 lives in `src/api/parity.rs`, and it is
...
to grow, alongside the layer list it already holds (§AR-001-layers.2).
