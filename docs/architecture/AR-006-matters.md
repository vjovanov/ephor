...
The nouns of [§FS-007-matters](../../requirements.md#fs-007-matters-the-feed-is-made-of-matters-and-a-matter-knows-why-it-is-there) as data. These types are core-layer
...
- `Matter { key, kind, placement, state, links, discussions, events,
  ...
  ([§FS-007-matters.1](../../requirements.md#1-a-matter-is-a-subject-with-a-stated-identity)): `gh:owner/repo#123`, `ticket:GR-73955`, a store's
...
- `Discussion { channel, messages, needs_response }`;
  ...
  where a channel tracks one ([§FS-003-feed-categories.4](../../requirements.md#4-a-conversation-is-answered-in-whatever-form-the-forge-recorded-it)).
- `Channel { id, capabilities }` — react, tick, reply
  ([§FS-007-matters.4](../../requirements.md#4-a-channel-says-what-it-can-do)).
- `Event { kind, time, payload }` — gate counts per repository, state
  transitions, check results ([§FS-007-matters.5](../../requirements.md#5-an-event-moves-state-and-resurfacing-names-its-reason)).
...
([§FS-003-feed-categories.5](../../requirements.md#5-one-subject-is-one-row-however-many-sources-reported-it)). The fingerprint digests state, each
...
([§FS-005-dispatch.5](../../requirements.md#5-an-item-that-moved-reopens-its-work), [§FS-007-matters.5](../../requirements.md#5-an-event-moves-state-and-resurfacing-names-its-reason)).
...
of [§FS-005-dispatch.2](../../requirements.md#2-the-ticket-carries-what-ephor-knows-not-a-link-to-it) — in two renderings from one source: prose for the
ticket, `EPHOR_*` identifiers for programs ([§FS-005-dispatch.8](../../requirements.md#8-the-ticket-carries-the-item-as-data-not-only-as-prose)), the
