...
reader pressing anything (§FS-005-dispatch.24). Two shapes give that: keep a
...
§FS-005-dispatch.20 already describes. Nothing about the run changes — the
...
It re-reads the world every time (§FS-005-dispatch.15), and the live-run
...
the work (§FS-005-dispatch.4). A root whose start failed is skipped for a
...
liveness signal every surface here reads (§FS-005-dispatch.15). A supervisor
...
root is the walk §FS-005-dispatch.15.1 rules out on every tick.

A run started by a sweep has nobody watching its first moments. Where the
launcher itself fails this is reported (§FS-005-dispatch.24), but a run that
