...
forest root (§FS-006-project-interface.2); a project that places nothing
...
the watched project is exactly what §REQ-001-boundary.3 forbids — if a
...
longer costless to watch (§GOAL-005-costless). The manifest therefore only
...
chosen, while a recipe (§FS-005-dispatch.1) spends the person's agent time
on its own match — and a repository does not get to spend the person's
agent time (§FS-006-project-interface.2). What dispatches on its own is
