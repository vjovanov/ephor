...
(§GOAL-004-handover). A runtime that can read a feed but cannot post the
...
(§GOAL-003-nothing-lost): a fact that can only be seen by a person sitting
...
an oversight (§REQ-001-boundary.1). A prompt that must be typed into is an
...
becoming part of it (§FS-010-doctor.3). A command that changes something
...
whatever a struct happened to serialize to (§GOAL-005-costless). Fields are
...
reasoning that makes §REQ-001-boundary.5 a build failure rather than a review
comment.
